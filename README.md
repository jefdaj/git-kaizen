...
When you find yourself doing something repetitive with files, stop and try to
codify it as a script. You define the commands to run along with input and
output filename patterns.

Later, `file-kaizen` will find matching files and suggest running the script on
them too. As with a build system, the real magic is in chaining scripts
together. But this is easier to design! You have a growing menu of "reasonable
next steps" in different situations, and they tend to form bottom-up pipelines
almost on their own.
